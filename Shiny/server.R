...
# Begin code for First Tab, CrossTab:
  ...
  dfc3 <- eventReactive(input$clicks1, {data.frame(fromJSON(getURL(URLencode(gsub("\n", " ", 'skipper.cs.utexas.edu:5001/rest/native/?query="select PROVIDERSTATE, DRGDEFINITION, sum(sum_payments) as sum_payments, kpi as ratio, 
case
when kpi < "p1" then \\\'03 Low\\\'
when kpi < "p2" then \\\'02 Medium\\\'
else \\\'01 High\\\'
end kpi
from (select PROVIDERSTATE, DRGDEFINITION, sum(sum_payments) as sum_payments, kpi as ratio, 
sum_payments / sum_charges as kpi
from MEDICALDATA
group by PROVIDERSTATE, DRGDEFINITION);"
